...
let
  ...
  cfg = config.${namespace}.programs.cli.tools.git;
...
in
{
  options.${namespace}.programs.cli.tools.git = {
    ...
    signingKey = mkOpt (types.nullOr types.str) null "The key ID to sign commits with (optional).";
    ...
    programs = {
      git = {
        ...
        extraConfig = {
          ...
          # Creates remote branch automatically when pushing
          push = {
          ...
          # Parallel submodule fetching (default is 1)
          submodule.fetchJobs = 4;
        };

        # Configures the signing key, if defined
